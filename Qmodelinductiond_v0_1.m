function [gx_final,gy_final,inh_energy] = Qmodelinductiond_v0_1(im,struct,scale)
...
% FFT preparation
siz=size(newgx_toroidal_Iitheta(max(1,M+1-diam):min(3*M,2*M+diam),max(1,N+1-diam):min(3*N,2*N+diam)));
FFT_M_exc_conv=fftn(complex(M_exc_conv,0),siz);
FFT_M_inh_conv=fftn(complex(M_inh_conv,0),siz);
FFT_M_norm_conv=fftn(complex(M_norm_conv,0),size(newgx_toroidal_Iitheta));
...
I_norm=zeros(M,N,K);
   ...
   for k=1:K
        if t==1
			  FFT_newgx_toroidal_Iitheta=fftn(complex(newgx_toroidal_Iitheta(max(1,M+1-diam):min(3*M,2*M+diam),max(1,N+1-diam):min(3*N,2*N+diam)),0));
			  
			  kk=real(convolucio_optima_FFT(FFT_newgx_toroidal_Iitheta,FFT_M_exc_conv));
			  x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
			  kk=real(convolucio_optima_FFT(FFT_newgx_toroidal_Iitheta,FFT_M_inh_conv));
			  y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
		  else
			  FFT_newgx_toroidal_x_retall=fftn(complex(newgx_toroidal_x(max(1,M+1-diam):min(3*M,2*M+diam),max(1,N+1-diam):min(3*N,2*N+diam)),0));
			  kk=real(convolucio_optima_FFT(FFT_newgx_toroidal_x_retall,FFT_M_exc_conv));
			  x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
			  kk=real(convolucio_optima_FFT(FFT_newgx_toroidal_x_retall,FFT_M_inh_conv));
			  y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
		  end

		  FFT_newgx_toroidal_x=fftn(complex(newgx_toroidal_x,0));
% 		  FFT_M_norm_conv=fftn(complex(M_norm_conv,0),size(newgx_toroidal_x));
        kk=real(convolucio_optima_FFT(FFT_newgx_toroidal_x,FFT_M_norm_conv));
