...
for b=6:length(blocks) %for b=1:length(blocks)
    ...
    for bc=1:max(block_cond_list(block_list==b))
        ...
        sindexes=zeros(length(model_names),length(model_names));
        ...
        for i=1:length(fileslist)
            ...
            for mo=1:length(model_names) 
                smap_path=[smaps_path '/' model_names{mo} '/' image_name_noext '.png'];
                smap=normalize_minmax(im2double(imread(smap_path)));
                Cw=SIndex(smap,mask);
                sindexes(mo,contrast_list(i))=Cw;
            end
            
            Cw_gt=SIndex(dmap,mask);
            sindexes_gt(contrast_list(i))=Cw_gt;
