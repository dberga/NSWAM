...
% if nargin<6,ior_decay=[1 0.999 0.99 0.9 0.75 0.5 0.25 0]; end
...
if length(ior_decay)>1 && length(ior_std_angle)<2
    ...
    if visualize
    plot(ior_decay,mean_amplitude_diff_decay); ylabel('Saccade Amplitude Diff.'); xlabel('ior decay factor'); 
    plot(ior_decay,mean_distance_decay); ylabel('Saccade Landing Diff.'); xlabel('ior decay factor'); 
    end
end
...
if length(ior_std_angle)>1 && length(ior_decay)<2
    ...
    if visualize
    plot(ior_std_angle,mean_amplitude_diff_angle); ylabel('Saccade Amplitude Diff.'); xlabel('ior angle pix'); 
    plot(ior_std_angle,mean_distance_angle); ylabel('Saccade Landing Diff.'); xlabel('ior angle pix'); 
    end
end
