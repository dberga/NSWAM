function [img,img_out]=general_NCZLdXim(estimul, nomexperiment, nstripes)
% new OP
...
% wave.mida = size of the last wavelet plane to process (default 32)
% zli.scale2size_type = old dedi revisited
% epsilon=1 gives 2.^(s-1)
% epsilon=0.5 gives 1.^(s)
% zli.scale2size_epsilon (was 1.1)
% zli.kappay = kappa y (was 1.25) (was 2!!!) (default 1.0)
% zli.normal_output = rescaled maximum value of output data for Z.Li method
% (was 1.5)
% image.single_or_multiple if is a static stimulus or a dynamic stimulus
% (default = 1, single).
% zli.single = name of the stimulus
zli.scalesize_type=-1; % default =1
...
disp(strcat('Nstripes in Model Neurodynamic: ',num2str(nstripes))); 
...
% parameters for the differential equationi
zli.n_membr=10;  % number of membrane time constant; was 15 (default 10).
zli.n_iter=10;   % integration steps in the Euler integration scheme; (default 10).
...
% type of normalization (default scale)
% zli.normal_type='scale';
zli.normal_type='all';
% zli.normal_type='absolute';

% reduce J and W (default 1)
zli.reduccio_JW=1;
...
% % Creacio de l'estimul dinamic (Rossi Paradiso)
% % period/wavelength oscillation of the dynamical stimulus (dynamical case only)
% Hz=2;
% zli.nb_periods=zli.n_membr/100*Hz;  % was 3 (default 5)
% zli.osc_wv=zli.nb_periods*(2*pi/zli.n_membr); % (default zli.nb_periods*(2*pi/zli.n_membr))

% boundary conditions default zli.boundary='mirror'
% zli.boundary='mirror';  % or 'wrap'

% power of the denominator in the normalization step 
zli.normalization_power=2; % (default 2)
...
if nargin > 1
    if (size(tmp, 4)>1)
        compute.dynamic=1; % 0 stable/1 dynamic stimulus (default 0)
        zli.n_membr=size(tmp,4);  % number of membrane time constant. One membrane time per frame of video;
...
compute.multiparameters=0; % 0 for the first parameter of the list/ 1 for all the parameters (default 0)

% use fft for speed
compute.use_fft=1; % (default 1)
% avoid circshift for speed
compute.avoid_circshift_fft=1; % (default 1)
% debug (display control values)
compute.XOP_DEBUG=0; % (default 0)
% debug (display scale interaction information)
compute.scale_interaction_debug=0; % This doesn't appear in the default parameters
% impose activity to a given unit
compute.XOP_activacio_neurona=0; % This doesn't appear in the default parameters
% high dynamic range
compute.HDR=0; %This doesn't appear in the default parameters
...
if nargin==0
    image.single_or_multiple=1; % 1 for single, 2 for multiple (default 1)
    ...
    % image.single='Assim_Verd_Blau_256';
    % image.single='Assim_Verd_Blau_256edited';
    % image.single='Contrast_Yellow_Green_128';
    ...
    % %
    %
    % image.gamma=2.4; % gamma correction (default 2.4)
    % image.updown=[1]; % up/downsample ([1,2]) (default [1])
...
if compute.dynamic==0 
	...
	elseif image.single_or_multiple==2 % three stimuli
        ...
        disp('Estimul multiple');
    else
        error('Error: variable image.single_or_multiple out of range [1,2]');
	end
end 
...
if compute.dynamic==0 % static
            if image.single_or_multiple==1
                for n=1:Nparam;
                    [img,img_out]=NCZLd(tmp,strct, nstripes);
                    % plot/no plot
...
%                        figure('Name',imageoutname)
            ...
            elseif image.single_or_multiple==2
                for n=1:Nparam;
                        [img1,img_out1]=NCZLd(tmp1,strct, nstripes);
                        [img2,img_out2]=NCZLd(tmp2,strct, nstripes);
                        [img3,img_out3]=NCZLd(tmp3,strct, nstripes);
                        % plot/no plot
                        if display_plot.plot_io==1
                            % get the name
                            str_delta=num2str(zli.Delta);
                        ...
                        if display_plot.store==1
                            save([struct.compute.outputstr '' imageoutname1],'img_out1')
                            save([struct.compute.outputstr '' imageoutname2],'img_out2')
                            save([struct.compute.outputstr '' imageoutname3],'img_out3')
                        end    
...
% 		end
    ...
    error('Error: variable compute.dynamic out of range [1,2]');
end
