...
% 		for ff=1:niter_wav
			...
			disp(['gabor_HMA n_scales:' int2str(n_scales)]);
% 		end
   otherwise
...
if(struct.display_plot.plot_io==1)
	...
	Mostrar_curv_video(curv,n_scales,1, struct.display_plot.y_video, struct.display_plot.x_video);
end
...
%        for orient=1:n_orient
			for ff=1:niter
             curv_final{ff}{scale}=out{scale-ini_scale+1}{ff}{scale};
			end
%        end
    end
    destroy(job);
...
if(struct.display_plot.plot_io==1)
	Mostrar_curv_video(curv_final,n_scales,1, struct.display_plot.y_video, struct.display_plot.x_video);
end
