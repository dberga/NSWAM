function [gx_final,gy_final,inh_energy] = Qmodelinduction_v3_4(im,curvdt,n_iter,r,boundary,de,di)
...
% % for a one-dimensional stimulus
...
Delta=10; % maximum radius of the area of influence
...
ver=0;
M_exc_conv=kappa*0.126.*exp(-d.^2/de);
if ver
...
M_inh_conv=kappa*0.126.*exp(-d.^2/di);
if ver
...
% den=5;
% inv_den=1/5;  % warning, the square comes below (was 25 in version v3)!
% two_dimensional
...
%                 kk=convn(newgx_toroidal_x,M_norm_conv,'same');
                ...
                % v4: only use the useful borders in the mirrored image

%                 if t==1
%                    kk=convn(newgx_toroidal_Iitheta(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_exc_conv,'same');
%                    x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
%                    kk=convn(newgx_toroidal_Iitheta(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_inh_conv,'same');
%                    y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
%                 else
%                    kk=convn(newgx_toroidal_x(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_exc_conv,'same');
%                    x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
%                    kk=convn(newgx_toroidal_x(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_inh_conv,'same');
%                    y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
%                 end
%                 
%                 kk=convn(newgx_toroidal_x,M_norm_conv,'same');
                ...
                if t==1
                   kk=convolucio_optima(newgx_toroidal_Iitheta(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_exc_conv);
                   x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
                   kk=convolucio_optima(newgx_toroidal_Iitheta(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_inh_conv);
                   y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
                else
                   kk=convolucio_optima(newgx_toroidal_x(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_exc_conv);
                   x_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
                   kk=convolucio_optima(newgx_toroidal_x(M+1-diam:2*M+diam,N+1-diam:2*N+diam),M_inh_conv);
                   y_int(:,:,k)=kk(diam+1:end-diam,diam+1:end-diam);
                end
