function [img_out,iFactor] = NCZLd_channel(img_in,struct,channel)
...
for scale=ini_scale:fin_scale
    ...
    % no parallel
        ...
        % (that in NCZLd_channel_orient and Qmodelinductiond_v0_0)
		  [curvtmp_final,iFactortmp]=NCZLd_channel_orient(curv,struct,scale);
%         curvs_final=curvs; % No faig res
		for ff=1:niter
			curv_final{ff}{scale}=curvtmp_final{ff};
            iFactor{ff}{scale}=iFactortmp{ff};
		end
