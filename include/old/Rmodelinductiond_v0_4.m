function [gx_final] = Rmodelinductiond_v0_4(Iitheta,struct)
...
for s=1:n_scales
	...
	if radius_sc >0
		J=zeros(diam(s),diam(s),2*radius_sc+1,K,K);
		W=zeros(diam(s),diam(s),2*radius_sc+1,K,K);
...
% 		half_size_filter{s}=[Delta(s) Delta(s) 0];
		for ss=1:2*radius_sc+1
			J(:,:,ss,:,:)=weight_scales(ss)*all_J{s}(:,:,:,:);
			W(:,:,ss,:,:)=weight_scales(ss)*all_W{s}(:,:,:,:);
		end
		all_J{s}=J;
		all_W{s}=W;
	end
	...
	if (use_fft)	
		...
		all_J_fft{s}=zeros(M+2*Delta(s),N+2*Delta(s),n_scales,K,K);
		all_W_fft{s}=zeros(M+2*Delta(s),N+2*Delta(s),n_scales,K,K);
		
		for ov=1:K
			for oc=1:K
				all_J_fft{s}(:,:,:,ov,oc)=fftn(all_J{s}(:,:,:,ov,oc),[M+2*Delta(s),N+2*Delta(s),n_scales]);
				all_W_fft{s}(:,:,:,ov,oc)=fftn(all_W{s}(:,:,:,ov,oc),[M+2*Delta(s),N+2*Delta(s),n_scales]);
			end
		end
...
% 				x_ei(:,:,:,oc)=sum(restr_sum_scale_newgy_toroidal_y.*repmat(w,[M,N,n_scales,1]),4)-restr_sum_scale_newgy_toroidal_y(:,:,:,oc);
				...
				for ov=1:K  % loop over all the orientations given the central (reference orientation)
					...
					if (use_fft)
						for s=1:n_scales
							x_tmp=zeros([size(newgx_toroidal_x{radius_sc+s}(:,:,ov) n_scales]));
							x_tmp=newgx_toroidal_x{radius_sc+s}(:,:,ov);
...
% 			radi=scale2size(s-radius_sc,zli.scale2size_type);
			...
			kk=convolucio_optima(sum_newgx_toroidal_x(Delta_ext(s)+1-radi:Delta_ext(s)+M+radi,Delta_ext(s)+1-radi:Delta_ext(s)+N+radi,:),M_norm_conv{s-radius_sc},radi,0); % Xavier. El filtre diria que ha d'estar normalitzat per tal de calcular el valor mig
			I_norm(:,:,s-radius_sc,:)=kk(radi+1:M+radi,radi+1:N+radi,:);
		end
