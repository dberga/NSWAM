function [gx_final] = Rmodelinductiond_v0_2(Iitheta,struct)
...
% 		end
	...
	disp(['t_iter: ' int2str(t_iter)]);
		...
		kk_tmp1=zeros(size(newgx_toroidal_x(:,:,1,:))); % El dia que ho generalitzem haura de ser 1:radius_sc
		kk_tmp2=kk_tmp1;
		kk_tmp1_y=zeros(size(newgy_toroidal_y(:,:,1,:))); % El dia que ho generalitzem haura de ser 1:radius_sc
		kk_tmp2_y=kk_tmp1_y;
		
		for i=1:radius_sc+1
			kk_tmp1=kk_tmp1+border_weight(i)*newgx_toroidal_x(:,:,radius_sc+i,:);
			kk_tmp2=kk_tmp2+border_weight(i)*newgx_toroidal_x(:,:,n_scales+radius_sc-(i-1),:);
			kk_tmp1_y=kk_tmp1_y+border_weight(i)*newgy_toroidal_y(:,:,radius_sc+i,:);
			kk_tmp2_y=kk_tmp2_y+border_weight(i)*newgy_toroidal_y(:,:,n_scales+radius_sc-(i-1),:);
		end
...
% 		sum_newgx_toroidal_x=convolucio_optima(newgx_toroidal_x,scale_filter_other,0,0); 
		sum_newgx_toroidal_x=convolucio_optima(newgx_toroidal_x,scale_filter,0,0); 
		sum_newgx_toroidal_x_sc=sum(sum_newgx_toroidal_x(:,:,radius_sc+1:radius_sc+n_scales,:),4);
		for o=1:K
% 			sum_newgx_toroidal_x(:,:,radius_sc+1:radius_sc+n_scales,o)=sum_newgx_toroidal_x_sc-sum_newgx_toroidal_x(:,:,radius_sc+1:radius_sc+n_scales,o); % Xavier. Hem de restar la propia orientacio
			sum_newgx_toroidal_x(:,:,radius_sc+1:radius_sc+n_scales,o)=sum_newgx_toroidal_x_sc;
		end
		
		kk=convolucio_optima(sum_newgx_toroidal_x(Delta-1:M+Delta+2,Delta-1:N+Delta+2,radius_sc+1:radius_sc+n_scales,:),M_norm_conv,0,0); % Xavier. El filtre diria que ha d'estar normalitzat per tal de calcular el valor mig
% 		kk=convolucio_optima(sum_newgx_toroidal_x(Delta-1:M+Delta+2,Delta-1:N+Delta+2),M_norm_conv*inv_den,0,0); % Xavier. El filtre diria que ha d'estar normalitzat per tal de calcular el valor mig
		I_norm=kk(2+1:M+2,2+1:N+2,:,:);
		I_norm=-2*(I_norm*inv_den).^r;

		if XOP_DEBUG
			show_x_y(fig,x,y,I_norm);
% 			I_norm=I_norm*0; % Xavier. I_norm=0;
		...
		% (1) excitatory neurons
			...
			+var_noise*rand(M,N,n_scales,K));   % neural noise (comment for speed)
% 				+0.85);             % spontaneous firing rate?
