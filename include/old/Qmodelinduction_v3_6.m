function [gx_final,gy_final,inh_energy] = Qmodelinduction_v3_6(im,curvdt,n_iter,r,boundary,de,di,genpar,scale)

%Delta=15; % maximum radius of the area of influence
% size_orig=size(im);
% Delta=sqrt(max(de,di))*3;

% padding=ceil(Delta/min(size(im)))+1
% im=repmat(im,padding,padding);

% modified 29_11_11 (added: genpar (general parameter structure))
kappa1=genpar.kappa1;
kappa1=kappa1(scale);
kappa2=genpar.kappa2;
kappa2=kappa2(scale);
...
% 	kkx=gx_final(1:size_orig(1),1:size_orig(2),:,:);
% 	kky=gy_final(1:size_orig(1),1:size_orig(2),:,:);
% 	gx_final=kkx;
% 	gy_final=kky;
