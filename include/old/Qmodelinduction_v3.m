function [gx_final,gy_final,inh_energy] = Qmodelinduction_v3(im,curvdt,n_iter,r,boundary,de,di)
       ...
       I_norm=zeros(M,N,K);
        ...
        for i=1:M
            for j=1:N
                ...
                if t==1
                   x_int(i,j,k)=sum(sum(sum(circshift(M_exc,[i-1,j-1,k-1]).*newgx(toroidal_Iitheta))));
                   y_int(i,j,k)=sum(sum(sum(circshift(M_inh,[i-1,j-1,k-1]).*newgx(toroidal_Iitheta))));
                else
                   x_int(i,j,k)=sum(sum(sum(circshift(M_exc,[i-1,j-1,k-1]).*newgx(toroidal_x))));
                   y_int(i,j,k)=sum(sum(sum(circshift(M_inh,[i-1,j-1,k-1]).*newgx(toroidal_x))));
                end
                ...
                I_norm(i,j,k)=-2*(sum(sum(sum(circshift(M_normalization,[i-1,j-1,k-1]).*newgx(toroidal_x))))/den)^r;
            end
        end    
