function [gx_final] = Rmodelinductiond_v0_3_1(Iitheta,struct)
...
% 		half_size_filter{s}=[Delta(s) Delta(s) radius_sc];
	...
	if (use_fft)	
		...
		M_norm_conv_fft{s}=fftn(M_norm_conv{s},[M+2*radi(1),N+2*radi(2),K]);
...
% 			kk=convolucio_optima(sum_newgx_toroidal_x(Delta_ext(s)+1-radi(1):Delta_ext(s)+M+radi(1),Delta_ext(s)+1-radi(2):Delta_ext(s)+N+radi(2),:),M_norm_conv{s-radius_sc},0,0); % Xavier. El filtre diria que ha d'estar normalitzat per tal de calcular el valor mig
			kk=convolucio_optima(sum_newgx_toroidal_x(Delta_ext(s)+1-radi(1):Delta_ext(s)+M+radi(1),Delta_ext(s)+1-radi(2):Delta_ext(s)+N+radi(2),:),M_norm_conv_fft{s-radius_sc},despl,1); % Xavier. El filtre diria que ha d'estar normalitzat per tal de calcular el valor mig
