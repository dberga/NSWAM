function [ mean_distance, std_distance, distances ] = pp_slanding( scanpath1, scanpath2, ff_flags )
    ...
    if iscell(scanpath1) && iscell(scanpath2)
        for p=1:length(scanpath1)
            [all_mean_distance{p},all_std_distance{p},all_distances{p}]=accumsl(scanpath1{p},scanpath2{p},ff_flags);
        end
        for p=1:length(scanpath1)
           if ~exist('distances')
    ...
    elseif iscell(scanpath1) && ~iscell(scanpath2)
        for p=1:length(scanpath1)
            [all_mean_distance{p},all_std_distance{p},all_distances{p}]=accumsl(scanpath1{p},scanpath2,ff_flags);
        end
        for p=1:length(scanpath1)
           if ~exist('distances')
               distances=cell(1,length(all_distances{p}));
    ...
    elseif ~iscell(scanpath1) && iscell(scanpath2)
        for p=1:length(scanpath2)
            [all_mean_distance{p},all_std_distance{p},all_distances{p}]=accumsl(scanpath1,scanpath2{p},ff_flags);
        end
        for p=1:length(scanpath2)
           if ~exist('distances')
               distances=cell(1,length(all_distances{p}));
        ...
        [mean_distance,std_distance,distances]=accumsl(scanpath1,scanpath2,ff_flags);
    end
